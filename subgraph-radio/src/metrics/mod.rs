...
#[allow(dead_code)]
pub static MAX_STAKE_POI: Lazy<GaugeVec> = Lazy::new(|| {
    let m = GaugeVec::new(
        Opts::new("max_stake_poi", "Highest stake-backed POI")
            .namespace("graphcast")
            .subsystem("subgraph_radio"),
        &["deployment_hash"],
    )
    .expect("Failed to create max_stake_poi gauge");
