...
pub async fn local_comparison_point(
    ...
    db: SqlitePool,
) -> Result<(u64, u32), ComparisonError> {
    ...
    let min_attestation = local_attestations
        ...
        .min_by_key(|attestation| {
            attestation
                .timestamp
                .first()
                .map_or((i64::MAX, i64::MAX), |&timestamp| {
                    let block_number_as_i64 = attestation.block_number as i64;
                    (block_number_as_i64, timestamp.into())
                })
        });
    ...
    let comparison_point = min_attestation
        .map(|attestation| {
            let timestamp =
                *attestation.timestamp.first().unwrap_or(&0) as i64 + collect_window_duration;
            (attestation.block_number, timestamp as u32)
        })
...
pub async fn save_local_attestation(
    ...
    let timestamp: Result<u32, _> = Utc::now().timestamp().try_into();
    ...
    insert_local_attestation(pool, attestation)
        .await
        .map_err(|e| OperationError::Database(DatabaseError::from(e)))
}
