...
use crate::entities::{get_local_attestations_by_identifier, insert_local_attestation};
...
impl Attestation {
    pub fn new(
        ...
        Attestation {
            ppoi,
            stake_weight: stake_weight as u64,
            senders,
            sender_group_hash,
            timestamp,
            identifier,
...
pub async fn local_comparison_point(
    ...
    let local_attestations = get_local_attestations_by_identifier(&db, id).await?;
    let remote_messages = get_remote_ppoi_messages_by_identifier(&db, id).await?;
    ...
    let min_attestation = local_attestations
        ...
        .min_by_key(|attestation| {
            attestation
                .timestamp
                .first()
                .map_or(i64::MAX, |&timestamp| {
                    let block_number_as_i64 = attestation.block_number as i64;
                    block_number_as_i64 * i64::MAX + timestamp
                })
        });

    let comparison_point = min_attestation.map(|attestation| {
...
pub async fn save_local_attestation(
    ...
    insert_local_attestation(pool, attestation).await
}
