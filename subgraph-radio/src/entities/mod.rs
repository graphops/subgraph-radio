...
pub async fn insert_local_attestation(
    ...
    for sender in &new_attestation.senders {
        ...
        sqlx::query(insert_sender_query)
            .bind(new_attestation.block_number as i64)
            .bind(&new_attestation.identifier)
            .bind(&new_attestation.ppoi) // Include ppoi value
            .bind(sender)
            .execute(pool)
            .await?;
    }
...
#[cached(
    key = "String",
    convert = r#"{ "attestations_cache_key".to_string() }"#
)]
pub async fn fetch_and_cache_attestations(pool: &SqlitePool) -> Vec<Attestation> {
    let basic_attestations = match sqlx::query!(
        r#"
        SELECT 
            identifier, 
            block_number, 
    ...
    for attestation in basic_attestations {
        ...
        .await
        {
            Ok(result) => result.into_iter().map(|record| record.sender).collect(),
            Err(_) => Vec::new(),
        };

        let block_number = attestation.block_number as i64;

        let timestamps = match sqlx::query!(
            r#"
            SELECT timestamp
            FROM local_attestation_timestamps
            WHERE attestation_identifier = ? AND attestation_block_number = ?
        ...
        .await
        {
            Ok(result) => result
                .into_iter()
                .map(|record| record.timestamp as i64)
                .collect(),
            Err(_) => Vec::new(),
        };
...
pub async fn get_all_local_attestations(pool: &SqlitePool) -> Result<Vec<Attestation>, SqlxError> {
    Ok(fetch_and_cache_attestations(pool).await)
}
...
pub async fn delete_outdated_local_attestations(
    ...
    sqlx::query(delete_senders)
        .bind(identifier)
        .bind(block_number as i64) // Cast to match the SQL type
        .execute(pool)
        .await?;
...
pub async fn get_all_upgrade_intent_messages(
    pool: &SqlitePool,
...
pub async fn get_comparison_results(pool: &SqlitePool) -> Result<Vec<ComparisonResult>, SqlxError> {
    let rows = sqlx::query!(
        r#"
    ...
    for row in rows {
        ...
        results.push(ComparisonResult {
            ...
            result_type: ComparisonResultType::from_str(&row.result_type).unwrap(),
            local_attestation,
            attestations,
        });
    }
...
pub async fn get_comparison_results_by_type(
    pool: &SqlitePool,
    result_type: &str,
) -> Result<Vec<ComparisonResult>, SqlxError> {
    let rows = sqlx::query!(
        r#"
        SELECT deployment, block_number, result_type, local_attestation_json, attestations_json
        FROM comparison_results
...
pub async fn get_comparison_results_by_deployment(
    pool: &SqlitePool,
    deployment_hash: &str,
) -> Result<Option<ComparisonResult>, SqlxError> {
    let row = sqlx::query!(
        r#"
        SELECT deployment, block_number, result_type, local_attestation_json, attestations_json
        FROM comparison_results
