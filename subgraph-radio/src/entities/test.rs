...
mod tests {
    ...
    use crate::{
        create_test_db,
        entities::{
            get_all_local_attestations, get_all_upgrade_intent_messages, get_comparison_results,
            get_comparison_results_by_deployment, get_local_attestations_by_identifier,
            get_remote_ppoi_messages, insert_local_attestation, insert_remote_ppoi_message,
    ...
    async fn test_db_basic() {
        ...
        let upgrade_intent_messages = get_all_upgrade_intent_messages(&pool).await.unwrap();
    ...
    async fn handle_comparison_result_change_result_type() {
        ...
        let _ = save_comparison_result(&pool, &old_result);
        let _ = handle_comparison_result(&pool, &new_result).await;
    ...
    async fn upgrade_ratelimiting() {
        ...
        let msgs = get_all_upgrade_intent_messages(&pool).await.unwrap();
        println!("{:?}", msgs);

        let msg = msgs.iter().find(|msg| msg.subgraph_id == test_id).unwrap();
