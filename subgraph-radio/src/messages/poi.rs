...
use crate::entities::{
    count_remote_ppoi_messages, get_local_attestation, insert_local_attestation,
    insert_remote_ppoi_message,
};
...
pub async fn poi_message_comparison(
    ...
    let (compare_block, collect_window_end) =
        match local_comparison_point(&id, collect_window_duration, db.clone()).await {
            Ok(Some((block, window))) if time >= window => (block, window),
            Ok(Some((block, _window))) => {
            ...
            Ok(None) => {
                // Construct error for lack of local attestations
                return Err(OperationError::CompareTrigger(
                    id,
                    0,
                    "No local attestations to compare".to_string(),
                ));
            }
            Err(e) => {
